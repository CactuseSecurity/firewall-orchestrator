...
do
   ...
   next_mgm_to_delete=$(psql -qtAX -d isodb -c "select mgm_id from ($sql_code) AS old_mgm_ids limit 1")
   ...
   echo "executing: time psql -qtAX -d isodb -c delete from management where mgm_id=$next_mgm_to_delete"
   #time psql -d isodb -c "delete from management where mgm_id='$next_mgm_to_delete'"
done
