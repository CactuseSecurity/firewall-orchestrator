...
use CACTUS::ISO::import;			# Import-Funktionen (Parser, etc.)
...
my $iso_workdir	= &read_config('iso_workdir') . "/$mgm_id";
my $archive_dir	= &read_config('archive_dir');
my $cfg_dir	= "${iso_workdir}/cfg";
...
($error_count_local, $error_str_local, $mgm_name, $dev_typ_id, $obj_file_base,$obj_file,$user_file_base,$user_file,$rule_file_base,$rule_file,
		...
		&get_import_infos_for_mgm($mgm_id, $iso_workdir, $cfg_dir);
...
if (!$error_count_global) {
	require "CACTUS/ISO/import/$hersteller.pm"; 	# load the matching parser at run time
	$rulebases = &get_rulebase_names($mgm_id, $CACTUS::ISO::dbdriver, $iso_database, $iso_srv_host, $iso_srv_port, $iso_srv_user, $iso_srv_pw);
	if (!$do_not_copy) {
		rmtree($iso_workdir); make_path($iso_workdir,{mode => 0700}); make_path($cfg_dir,{mode => 0700});
		$error_count_global = &error_handler_add($current_import_id, $error_level = 2, "copy-ssh-keys-failed",
				$error_count_local = &put_ssh_keys_in_place ($iso_workdir, $ssh_public_key, $ssh_private_key), $error_count_global);
		...
		($error_count_local, $config_files_str) =
			&CACTUS::ISO::import::parser::copy_config_from_mgm_to_iso 
				($ssh_user, $ssh_hostname, $mgm_name, $obj_file_base, $cfg_dir, $rule_file_base,
					$iso_workdir, $audit_log_file, $prev_imp_time, $ssh_port, $config_path_on_mgmt, $rulebases);	# TODO: add use_scp parameter
		...
		&iconv_config_files_2_utf8($config_files_str, $iso_workdir);	# convert config files from latin1 to utf-8
		
		$new_md5sum = &calc_md5_of_files($config_files_str, $iso_workdir); # iso_workdir is here tmpdir
	...
	if (!$error_count_global) {
		if ($new_md5sum ne $stored_md5sum_of_last_import || $no_md5_checks) {
			...
			if (!$error_count_global) {
				...
				if ($clear_all_rules) { find(\&empty_rule_files,$iso_workdir); } # rule csv files leeren, wenn initial import eines Devices erzwungen werden soll
				if ($clear_whole_mgm_config) { find(\&empty_config_files,$iso_workdir); } # alle csv files leeren, wenn initial import eines Managements erzwungen werden soll
				if (-e $csv_usr_file) { &iconv_2_utf8($csv_usr_file, $iso_workdir); }		# utf-8 conversion of user data
				$error_count_local = &fill_import_tables_from_csv($dev_typ_id,$csv_zone_file, $csv_obj_file, $csv_svc_file, $csv_usr_file, $rulebases, $iso_workdir, $csv_auditlog_file);
				if ($error_count_local) { # wenn ein Fehler beim csv-Import auftritt: einzeln parsen um alle Fehler zu sehen
					...
					if ($show_all_import_errors) {
						&fill_import_tables_from_csv_with_sql ($dev_typ_id,$csv_zone_file, $csv_obj_file, $csv_svc_file, $csv_usr_file, $rulebases, $iso_workdir, $csv_auditlog_file);
					}
				}
...
#					&read_user_client_classification_from_ldap ($error_count_local=1, $current_import_id);
			...
			if ($error_count_global) { output_txt ("Import of management $mgm_name (mgm_id=$mgm_id, import_id=$current_import_id): FOUND $error_count_global error(s)\n", 2); }
				...
				output_txt ("Import of management $mgm_name (mgm_id=$mgm_id, import_id=$current_import_id)" .
						", total time: " . sprintf("%.2fs",(time() - $first_start_time)) . "): no errors found" . (($changes eq '')?", no changes found":"") . "\n");
				&exec_pgsql_cmd_no_result("UPDATE import_control SET successful_import=TRUE WHERE control_id=$current_import_id"); # if no error occured - set import_control.successful_import to true
			}
		} else {
	...
	if (defined($save_import_results_to_file) && $save_import_results_to_file && ($error_count_global || $changes ne '')) { # if changes or errors occured: move config & csv to archive
		system ("${bin_path}mkdir -p $archive_dir; cd $iso_workdir; ${bin_path}tar cfz $archive_dir/${current_import_id}_`${bin_path}date +%F_%T`_mgm_id_$mgm_id.tgz .");
	}
	if (!$no_cleanup) { rmtree $iso_workdir; }
} else {
	&exec_pgsql_cmd_no_result("SELECT remove_import_lock($current_import_id)");   # this sets import_control.stop_time to now()
}
