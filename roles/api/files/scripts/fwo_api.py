...
def write_device_details(fwo_api_base_url, jwt, query_variables):
    device_mutation = """
        ...
        # }
    """
    api_call_result = call(fwo_api_base_url, jwt, device_mutation, query_variables=query_variables, role='admin')
    if 'data' in api_call_result and 'management' in api_call_result['data'] and len(api_call_result['data']['management'])>=1:
        ...
        logging.error('did not succeed in writing device details to API')
        sys.exit(1)
