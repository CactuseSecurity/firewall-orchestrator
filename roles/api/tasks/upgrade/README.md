...
Each file here is named after the product version it upgrades an installation *to*, and
`run-upgrades.yml` applies every file whose version is above the installed one. Add a new
...
file: the `fileglob` lookups in `run-upgrades.yml` warn on every run about a path that
does not exist.
