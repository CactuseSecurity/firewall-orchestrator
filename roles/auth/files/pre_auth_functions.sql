...
BEGIN
    ...
    IF b_can_view_all_devices THEN
        ...
        END LOOP;
    ELSE
        FOR i_dev_id, v_dev_name IN SELECT device_id, dev_name FROM role JOIN tenant_to_device USING (role_id) LEFT JOIN device ON (tenant_to_device.dev_id=device.dev_id) WHERE role_id=i_role_id
        LOOP
            RETURN NEXT ROW (i_dev_id, i_dev_name);
        END LOOP;
    END IF;
...
BEGIN
    ...
    IF b_can_view_all_devices THEN
        ...
        END LOOP;
    ELSE
        FOR i_dev_id IN SELECT device_id FROM role JOIN tenant_to_device USING (role_id) WHERE role_id=i_role_id
        LOOP
            RETURN NEXT i_dev_id;
        END LOOP;
    END IF;
...
--    SELECT INTO b_can_view_all_devices bool_or(role_can_view_all_devices) FROM role_to_user JOIN role USING (role_id) WHERE role_to_user.user_id=i_user_id;
    ...
    IF b_can_view_all_devices THEN
        ...
        END LOOP;
    ELSE
        FOR i_mgm_id IN SELECT mgm_id FROM role JOIN tenant_to_device USING (role_id) JOIN device ON (tenant_to_device.device_id=device.dev_id) WHERE role.role_id=i_role_id
        LOOP
            RETURN NEXT i_mgm_id;
        END LOOP;
    END IF;
