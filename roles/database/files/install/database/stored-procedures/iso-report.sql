...
----------------------------------------------------
-- get_client_ip_filter(client_id) RETURNS VARCHAR
-- get_negated_client_ip_filter(client_id) RETURNS VARCHAR

-- get_obj_ids_of_filtered_ruleset(INTEGER[]) RETURNS SETOF INTEGER

-- get_rule_ids(device_id, zeitpunkt, client_id) RETURNS SETOF INTEGER
-- get_rule_ids(device_id, zeitpunkt, client_id, src_ip, dst_ip, any_ip, proto, port) RETURNS SETOF INTEGER --> for rulesearch

-- rule_src_contains_client_obj (rule_id, client_id) RETURNS BOOLEAN
-- rule_dst_contains_client_obj (rule_id, client_id) RETURNS BOOLEAN
-- obj_belongs_to_client (obj_id, client_id) RETURNS BOOLEAN
-- obj_neg_belongs_to_client (obj_id, client_id) RETURNS BOOLEAN

-- get_rule_src (rule_id, client_id) RETURNS SETOF INTEGER
-- get_rule_dst (rule_id, client_id) RETURNS SETOF INTEGER

-- get_client_relevant_changes(Client-ID, management-id, device-id, startzeit, stopzeit)
-- unterfunktionen von get_client_relevant_changes:
	-- get_svc_ids_of_client(client, array_of_rule_ids) RETURNS SETOF INTEGER
	-- get_user_ids_of_client(client, array_of_rule_ids) RETURNS SETOF INTEGER
	-- get_obj_ids_of_client(client, array_of_rule_ids) RETURNS SETOF INTEGER

----------------------------------------------------
-- FUNCTION:	get_client_relevant_changes
-- Zweck:		liefert zu einem Client alle Changes zurueck, die in fuer ihn relevanten sind
-- Parameter1:	Client-ID des zu betrachtenden Clients
...
--
CREATE OR REPLACE FUNCTION get_client_relevant_changes(INTEGER, INTEGER, INTEGER, TIMESTAMP, TIMESTAMP) RETURNS SETOF INTEGER AS $$
DECLARE
	i_client_id	ALIAS FOR $1;
	...
	v_sql		VARCHAR;
BEGIN
	-- delete content of temp table anf fill it again with all rule_ids of the requested client
	DELETE FROM temp_table_for_client_filtered_rule_ids;
	INSERT INTO temp_table_for_client_filtered_rule_ids SELECT rule_id FROM view_client_rules WHERE client_id=i_client_id;
	...
	v_sql := v_sql || '	AND	((change_element=''service'' and element_id in (select * from get_svc_ids_for_client())) ' ||
		'OR (change_element=''user'' and element_id in (select * from get_user_ids_for_client())) ' ||
		'OR (change_element=''object'' and element_id in (select * from get_obj_ids_for_client())) ' ||
		'OR (change_element=''rule'' and element_id in (select * from temp_table_for_client_filtered_rule_ids))' ||
...
----------------------------------------------------
-- FUNCTION:	get_svc_ids_of_client
-- Zweck:		liefert zu einem Client alle dienste zurueck, die in fuer ihn relevanten regeln vorkommen
-- Annahme:		die Menge der Regeln steht in temp_table_for_client_filtered_rule_ids
-- RETURNS:		Menge der Dienst-IDs (svc_id)
--
CREATE OR REPLACE FUNCTION get_svc_ids_for_client() RETURNS SETOF INTEGER AS $$
DECLARE
	r_svc				RECORD;
BEGIN		
	FOR r_svc IN
		...
		WHERE rule.rule_id IN (SELECT rule_id FROM temp_table_for_client_filtered_rule_ids)
...
----------------------------------------------------
-- FUNCTION:	get_user_ids_of_client
-- Zweck:		liefert zu einem Client alle User zurueck, die in fuer ihn relevanten regeln vorkommen
-- Annahme:		die Menge der Regeln steht in temp_table_for_client_filtered_rule_ids
-- RETURNS:		Menge der User-IDs (user_id)
--
CREATE OR REPLACE FUNCTION get_user_ids_for_client() RETURNS SETOF INTEGER AS $$
DECLARE
	r_user			RECORD;
BEGIN			 
	FOR r_user IN
...
----------------------------------------------------
-- FUNCTION:	get_obj_ids_of_client
-- Zweck:		liefert zu einem Client alle NW-Objekte zurueck, die in fuer ihn relevanten regeln vorkommen
-- Annahme:		die Menge der Regeln steht in temp_table_for_client_filtered_rule_ids
-- RETURNS:		Menge der object-IDs (obj_id)
--
CREATE OR REPLACE FUNCTION get_obj_ids_for_client() RETURNS SETOF INTEGER AS $$
DECLARE
	r_obj				RECORD;
BEGIN		
	FOR r_obj IN
...
----------------------------------------------------
-- FUNCTION:	get_client_ip_filter
-- Zweck:		liefert zu einem Client einen Filter-String
-- Parameter1:	Client-ID des zu betrachtenden Clients
-- RETURNS:		String mit booleschem Ausdruck fuer SQL-Where-Clause
--
CREATE OR REPLACE FUNCTION get_client_ip_filter(INTEGER) RETURNS VARCHAR AS $$
DECLARE
	i_client_id ALIAS FOR $1;
	v_filter VARCHAR;
	r_client_net RECORD;
BEGIN
	...
	ELSE 
		v_filter := '(';
		FOR r_client_net IN
			SELECT client_net_ip FROM client_network WHERE client_id=i_client_id
		LOOP
			v_filter := v_filter || ' obj_ip<<=' || E'\'' ||
				CAST (r_client_net.client_net_ip AS VARCHAR) || E'\'' || ' OR ' || E'\'' ||
				CAST (r_client_net.client_net_ip AS VARCHAR) || E'\'' || '<<=obj_ip OR';
		END LOOP;
		v_filter := v_filter || ' FALSE)';
--		RAISE INFO 'client-filter: %', v_filter;
	    RETURN v_filter;
	END IF;
END;
$$ LANGUAGE plpgsql;
...
$$ LANGUAGE plpgsql;
----------------------------------------------------
-- FUNCTION:	get_negated_client_ip_filter
-- Zweck:		liefert zu einem Client einen Filter-String
-- Zweck:		fuer ngegierte Regelteile (Quelle oder Ziel)
-- Parameter1:	Client-ID des zu betrachtenden Clients
-- RETURNS:		String mit booleschem Ausdruck fuer SQL-Where-Clause
--
CREATE OR REPLACE FUNCTION get_negated_client_ip_filter(INTEGER) RETURNS VARCHAR AS $$
DECLARE
	i_client_id ALIAS FOR $1;
	v_filter VARCHAR;
	r_client_net RECORD;
	...
	ELSE 
		...
		LOOP
			v_filter := v_filter || 'NOT(obj_ip<<=' || E'\'' ||
				CAST (r_client_net.client_net_ip AS VARCHAR) || E'\'' || ') AND NOT(' || E'\'' ||
				CAST (r_client_net.client_net_ip AS VARCHAR) || E'\'' || '<<=obj_ip) AND ';
		END LOOP;
...
BEGIN
	...
	LOOP
		FOR r_obj IN
			(
				(
--					SELECT get_rule_src AS obj_id FROM get_rule_src(r_rule.rule_id,i_import_id,i_client)
					SELECT get_rule_src AS obj_id FROM get_rule_src(r_rule.rule_id,i_client,t_time)
				)
				UNION
				(
--					SELECT get_rule_dst AS obj_id FROM get_rule_dst(r_rule.rule_id,i_import_id,i_client)
					SELECT get_rule_dst AS obj_id FROM get_rule_dst(r_rule.rule_id,i_client,t_time)
				)
...
BEGIN
	...
	LOOP
		FOR r_obj IN
			(
				SELECT get_rule_src_flat AS obj_id FROM get_rule_src_flat(r_rule.rule_id,i_client,t_time)
				UNION
				SELECT get_rule_dst_flat AS obj_id FROM get_rule_dst_flat(r_rule.rule_id,i_client,t_time)
			)
		LOOP
			RETURN NEXT r_obj.obj_id;
		END LOOP;
...
-- Zweck:		die in den Filterkriterien (Client und Zeitpunkt) gen�gen
...
DECLARE
    ...
    i_client ALIAS FOR $3;
    r_obj RECORD;
    v_sql_code VARCHAR;
    v_filter VARCHAR;
BEGIN
	IF NOT i_client IS NULL THEN
		v_filter := get_client_ip_filter(i_client);
...
/* -- alte Funktion mit Client-Filter
...
--	v_order_statement := ' ORDER BY dev_id';
	...
	ELSE -- Client-Filter
		...
		v_select_statement := 'SELECT rule_id FROM rule_order WHERE rule_id IN (' || v_sql_get_rules_with_client_src_ips 
			|| ' UNION ' ||	v_sql_get_rules_with_client_dst_ips || ') ' || ' GROUP BY rule_order.rule_id ';
...
----------------------------------------------------
-- FUNCTION:	rule_src_contains_client_obj
...
--    i_import_id ALIAS FOR $2;
    ...
    r_obj	RECORD; -- object
	v_client_ip_filter VARCHAR;
BEGIN
	IF is_rule_src_negated(i_rule_id) THEN
		v_client_ip_filter := get_negated_client_ip_filter(i_client_id);
	ELSE
		v_client_ip_filter := get_client_ip_filter(i_client_id);
	END IF;
...
--	INSERT INTO obj_id_table SELECT get_exploded_src_of_rule AS id FROM get_exploded_src_of_rule(i_rule_id);
	FOR r_obj IN EXECUTE
		...
		CAST(i_rule_id AS VARCHAR) || '))) AND (' || v_client_ip_filter || ')'
--		CAST(i_rule_id AS VARCHAR) || ',' || CAST(i_import_id AS VARCHAR) || '))) AND (' || v_client_ip_filter || ')'
	LOOP
...
-- FUNCTION:	rule_dst_contains_client_obj
...
----------------------------------------------------
-- FUNCTION:	obj_belongs_to_client
-- Zweck:		prueft, ob das NW-Objekt zum Client-Bereich gehoert
-- Parameter1:	obj_id
...
DECLARE
	...
	v_client_filter_ip_list VARCHAR;
BEGIN
	v_client_filter_ip_list := get_client_ip_filter(i_client_id);
--	RAISE INFO 'client: %', i_client_id;
--	RAISE INFO 'client_filter: %', v_client_filter_ip_list;
	FOR r_obj IN EXECUTE
		'SELECT obj_id FROM object WHERE (obj_id IN (SELECT * FROM explode_objgrp(' || i_obj_id ||
		'))) AND ('|| v_client_filter_ip_list || ')'
	LOOP
--		RAISE INFO 'obj: %', r_obj.obj_id;
		RETURN TRUE;
	END LOOP;
...
----------------------------------------------------
-- FUNCTION:	obj_neg_belongs_to_client
-- Zweck:		prueft, ob die Negation des NW-Objekts zum Client-Bereich gehoert
-- Parameter1:	obj_id
-- Parameter2:	client_id
...
DECLARE
    ...
    i_client_id ALIAS FOR $2;
    r_obj	RECORD;				-- zu pruefendes Objekt
	v_client_filter_ip_list_neg VARCHAR;
BEGIN
	v_client_filter_ip_list_neg := get_negated_client_ip_filter(i_client_id);
	FOR r_obj IN EXECUTE
		'SELECT obj_id FROM object WHERE (obj_id IN (SELECT * FROM explode_objgrp(' || i_obj_id ||
		'))) AND ('|| v_client_filter_ip_list_neg || ')'
	LOOP
		RETURN TRUE;
	END LOOP;
	RETURN FALSE;
...
--		RAISE NOTICE 'import: %', i_import_id;
	...
	ELSE
		-- do the filtering	
		IF rule_dst_contains_client_obj(i_rule_id, i_client_id) THEN -- alle Quellen anzeigen
			FOR r_obj IN
	   			SELECT obj_id FROM rule,rule_from WHERE rf_last_seen>=i_import_id AND rf_create<=i_import_id AND
					rule.rule_id=rule_from.rule_id AND rule.rule_id=i_rule_id
			LOOP
				RETURN NEXT r_obj.obj_id;
			END LOOP;
		ELSE -- filtern - nur Client-Objekte anzeigen
			IF is_rule_src_negated(i_rule_id) THEN
				FOR r_obj IN
		   			SELECT obj_id FROM rule,rule_from WHERE rf_last_seen>=i_import_id AND rf_create<=i_import_id AND
						rule.rule_id=rule_from.rule_id AND rule.rule_id=i_rule_id
				LOOP
					IF obj_neg_belongs_to_client(r_obj.obj_id, i_client_id) THEN
						RETURN NEXT r_obj.obj_id;
					END IF;
				END LOOP;
			ELSE
				FOR r_obj IN
		   			SELECT obj_id FROM rule,rule_from WHERE rf_last_seen>=i_import_id AND rf_create<=i_import_id AND
						rule.rule_id=rule_from.rule_id AND rule.rule_id=i_rule_id
				LOOP
					IF obj_belongs_to_client(r_obj.obj_id, i_client_id) THEN
						RETURN NEXT r_obj.obj_id;
					END IF;
				END LOOP;
			END IF;
...
BEGIN
	...
	ELSE -- do the filtering	
		IF rule_src_contains_client_obj(i_rule_id, i_client_id) THEN -- alle Quellen anzeigen
			FOR r_obj IN
	   			SELECT obj_id FROM rule,rule_to WHERE rt_last_seen>=i_import_id AND rt_create<=i_import_id AND
					rule.rule_id=rule_to.rule_id AND rule.rule_id=i_rule_id
			LOOP
...
BEGIN
	SELECT INTO i_mgm_id device.mgm_id FROM rule_order LEFT JOIN device USING (dev_id) WHERE rule_id=i_rule_id LIMIT 1;
	i_import_id := get_import_id_for_mgmt_at_time(i_mgm_id,t_time);
	IF i_client_id IS NULL OR rule_dst_contains_client_obj(i_rule_id, i_client_id) THEN
--		RAISE NOTICE 'import: %', i_import_id;
		FOR r_obj IN
	...
	ELSE -- filtern - nur Client-Objekte anzeigen
		IF is_rule_src_negated(i_rule_id) THEN
			FOR r_obj IN
				...
				WHERE rf_last_seen>=i_import_id AND rf_create<=i_import_id AND rule.rule_id=i_rule_id
			LOOP
				IF obj_neg_belongs_to_client(r_obj.objgrp_flat_member_id, i_client_id) THEN
					RETURN NEXT r_obj.objgrp_flat_member_id;
				END IF;
			END LOOP;
		ELSE
			FOR r_obj IN
				...
				WHERE rf_last_seen>=i_import_id AND rf_create<=i_import_id AND rule.rule_id=i_rule_id
			LOOP
				IF obj_belongs_to_client(r_obj.objgrp_flat_member_id, i_client_id) THEN
					RETURN NEXT r_obj.objgrp_flat_member_id;
				END IF;
			END LOOP;
		END IF;
...
BEGIN
	SELECT INTO i_mgm_id device.mgm_id FROM rule_order LEFT JOIN device USING (dev_id) WHERE rule_id=i_rule_id LIMIT 1;
	i_import_id := get_import_id_for_mgmt_at_time(i_mgm_id,t_time);
	IF i_client_id IS NULL OR rule_src_contains_client_obj(i_rule_id, i_client_id) THEN
--		RAISE NOTICE 'import: %', i_import_id;
		FOR r_obj IN
