	...
	ELSE 
		...
		IF (NOT i_obj IS NULL) THEN
			IF i_usr IS NULL THEN
				...
				EXCEPTION
					WHEN others THEN
						raise notice 'f_add_single_rule_from_element - INSERT rule_from without user - uncommittable state. Rolling back';
						raise notice '% %', SQLERRM, SQLSTATE;    
				END;
				BEGIN
					PERFORM import_rule_resolved_nwobj(i_mgm_id, i_rule_id, i_obj, i_current_import_id);
				EXCEPTION
					WHEN others THEN
						raise notice 'f_add_single_rule_from_element - rule_from without user import_rule_resolved_nwobj - uncommittable state. Rolling back';
						raise notice '% %', SQLERRM, SQLSTATE;    
				END;
			ELSE 
				BEGIN
				...
				EXCEPTION
					WHEN others THEN
						raise notice 'f_add_single_rule_from_element - INSERT rule_from with user - uncommittable state. Rolling back';
						raise notice '% %', SQLERRM, SQLSTATE;    
				END;
				BEGIN
					PERFORM import_rule_resolved_nwobj(i_mgm_id, i_rule_id, i_obj, i_current_import_id);
				EXCEPTION
					WHEN others THEN
						raise notice 'f_add_single_rule_from_element - rule_from with user import_rule_resolved_nwobj - uncommittable state. Rolling back';
						raise notice '% %', SQLERRM, SQLSTATE;    
				END;
				BEGIN
					PERFORM import_rule_resolved_usr(i_mgm_id, i_rule_id, i_usr, i_current_import_id);
				EXCEPTION
					WHEN others THEN
...
-- 		TODO: check if exactly one hit found
	...
	ELSE 
		...
		PERFORM import_rule_resolved_nwobj(i_mgm_id, i_rule_id,r_obj.obj_id, i_current_import_id);
	END IF;
	RETURN;
END;
$$ LANGUAGE plpgsql;
...
--		RAISE NOTICE 'Error: found duplicate service in rule: %', v_element;
	...
	ELSE
		...
		PERFORM import_rule_resolved_svc(i_mgm_id, i_rule_id,r_svc.svc_id, i_current_import_id);
	END IF;	
	RETURN;
END;
$$ LANGUAGE plpgsql;
