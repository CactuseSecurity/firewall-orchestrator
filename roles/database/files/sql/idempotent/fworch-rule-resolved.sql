    ...
    BEGIN -- catch
        ...
        ELSE
            ...
            IF is_obj_group(i_new_obj_id) THEN -- treat all group members as well
                ...
                FOR i_member IN SELECT objgrp_flat_member_id FROM objgrp_flat WHERE active AND objgrp_flat_id=i_new_obj_id
                LOOP
                    ...
                    IF i_new_obj_id <> i_member THEN 
                        RAISE NOTICE 'import_rule_resolved_nwobj 4b recurse I - i_mgm_id=%, i_rule_id=%, i_old_obj_id=%, i_new_obj_id=%, i_current_import_id=%, c_action=%, c_changelog_table=%', 
                            i_mgm_id, i_rule_id, i_old_obj_id, i_new_obj_id, i_current_import_id, c_action, c_changelog_table;
                        -- add new members:
                        PERFORM import_rule_resolved_nwobj(i_mgm_id, i_rule_id, NULL, i_member, i_current_import_id, c_action, c_changelog_table);
                    END IF;
                END LOOP;
                -- delete old member refs, TODO: this part must be tested
                FOR i_member IN SELECT objgrp_flat_member_id FROM objgrp_flat WHERE NOT active AND objgrp_flat_id=i_old_obj_id AND 
                    NOT (objgrp_flat_member_id IN (SELECT objgrp_flat_member_id AS old_member FROM objgrp_flat WHERE objgrp_flat_id=i_new_obj_id))
                LOOP
                    RAISE NOTICE 'import_rule_resolved_nwobj 4a group loop - i_mgm_id=%, i_rule_id=%, i_old_obj_id=%, i_new_obj_id=%, i_current_import_id=%, c_action=%, c_changelog_table=%', 
                        i_mgm_id, i_rule_id, i_old_obj_id, i_new_obj_id, i_current_import_id, c_action, c_changelog_table;
                    IF NOT i_old_obj_id IS NULL AND i_old_obj_id <> i_member THEN 
                        RAISE NOTICE 'import_rule_resolved_nwobj 4c recurse D/C - i_mgm_id=%, i_rule_id=%, i_old_obj_id=%, i_new_obj_id=%, i_current_import_id=%, c_action=%, c_changelog_table=%', 
                            i_mgm_id, i_rule_id, i_old_obj_id, i_new_obj_id, i_current_import_id, c_action, c_changelog_table;
        ...
        ELSE
            ...
            IF is_svc_group(i_new_obj_id) THEN -- treat all group members as well
                RAISE NOTICE 'import_rule_resolved_svc 4 group - i_mgm_id=%, i_rule_id=%, i_old_obj_id=%, i_new_obj_id=%, i_current_import_id=%, c_action=%, c_changelog_table=%', 
                    i_mgm_id, i_rule_id, i_old_obj_id, i_new_obj_id, i_current_import_id, c_action, c_changelog_table;
                FOR i_member IN
                        SELECT svcgrp_flat_member_id FROM svcgrp_flat WHERE active AND svcgrp_flat_id=i_new_obj_id
                    LOOP
        ...
        ELSE
            ...
            IF is_user_group(i_new_obj_id) THEN -- treat all group members as well
                RAISE NOTICE 'import_rule_resolved_usr 4 group - i_mgm_id=%, i_rule_id=%, i_old_obj_id=%, i_new_obj_id=%, i_current_import_id=%, c_action=%, c_changelog_table=%', 
                    i_mgm_id, i_rule_id, i_old_obj_id, i_new_obj_id, i_current_import_id, c_action, c_changelog_table;
                FOR i_member IN
                        SELECT usergrp_flat_member_id FROM usergrp_flat WHERE active AND usergrp_flat_id=i_new_obj_id
                    LOOP
