...
<ul>
    ...
    <li>Die Aktionen eines Statuswechsels im Workflow werden je Statuswechsel nur noch einmal ausgef&uuml;hrt. Ein versehentlich doppelt abgeschickter Statuswechsel verschickt keine zweite Email, erzeugt keinen zweiten externen Antrag und legt keinen zweiten Flow mehr an.</li>
...
<ul>
    ...
    <li>The actions of a workflow state change now run once per state change. A state change submitted twice by accident no longer sends a second email, raises a second external request or creates a second flow.</li>
