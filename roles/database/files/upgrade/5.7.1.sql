...
DO $do$ BEGIN
    IF EXISTS (SELECT tenant_id FROM tenant WHERE tenant_name='tenant1_demo') THEN
        ...
        END IF;
        IF NOT EXISTS (SELECT * FROM tenant_network LEFT JOIN tenant USING (tenant_id) WHERE tenant_name='tenant1_demo' and tenant_net_ip='10.0.0.48/29') THEN
            insert into tenant_network (tenant_id, tenant_net_ip, tenant_net_comment)
            VALUES ((SELECT tenant_id FROM tenant WHERE tenant_name='tenant1_demo'), '10.0.0.36/31', 'demo network for tenant 1') ON CONFLICT DO NOTHING;
        END IF;
        IF NOT EXISTS (SELECT * FROM tenant_network LEFT JOIN tenant USING (tenant_id) WHERE tenant_name='tenant1_demo' and tenant_net_ip='10.0.0.7/32') THEN
            insert into tenant_network (tenant_id, tenant_net_ip, tenant_net_comment)
            VALUES ((SELECT tenant_id FROM tenant WHERE tenant_name='tenant1_demo'), '10.0.0.7/32', 'demo network for tenant 1') ON CONFLICT DO NOTHING;
        END IF;
    END IF;
