...
//			$this->report_time = date('Y-m-d H:i:s');
		...
		public function reportExists($db_type, $connect_str, $report_type, $dev_id, $time1, $time2, $client_id, $collection, $session) {
			$log = new LogConnection();
//			$log->log_debug("db-report-storage::reportExists params: $db_type, $connect_str, $report_type, $dev_id, $time1, $time2, $client_id, $collection");
			if ($db_type=='mongo') {
				$filter = [
...
//						"client_id" => $client_id,
			...
			if ($db_type=='postgres') {
				...
				$client_id_filter = ' (TRUE) ';
				if (isset($client_id)) { $client_id_filter = 'client_id=$client_id'; }
				...
				if (preg_match("/config/", $report_type)) { // standard config report
					...
					if (isset($result->data[0]['get_import_id_for_dev_at_time'])) {
						...
						if (isset($client_id) and $client_id!='') {
							if (preg_match("/^null$/i", $client_id)) $sql_code .= " AND client_id IS NULL";
		...
		public function readReport($db_type, $connect_str, $report_type, $dev_id, $time1, $time2, $client_id, $collection, $session) { 
			//	retrieve report from databse
			$log = new LogConnection();
//			$log->log_debug("db-report-storage::readReport params: $db_type, $connect_str, $report_type, $dev_id, $time1, $time2, $client_id, $collection");
			if ($db_type=='mongo') {
				$filter = [
						"report_type" => $report_type,
						"dev_id" => $dev_id,
...
//						"client_id" => $client_id,
			...
			if ($db_type=='postgres') {
				...
				$db_connection = new DbConnection(new DbConfig($session["dbuser"], $session["dbpw"]));
				$client_id_filter = ' (TRUE) ';
				if (isset($client_id)) { $client_id_filter = 'client_id=$client_id'; } // TODO
		...
		public function dumpReport($db_type, $connect_str, $report_type, $dev_id, $time1, $time2, $client_id, $collection, $session) { 
...
//				if (isset($result->data[0]['get_import_id_for_dev_at_time'])) {			
				...
				if (isset($first_import_id)) {
					$sql_code = 
						"INSERT INTO report (report_typ_id, start_import_id, stop_import_id, dev_id, report_start_time, report_end_time, client_id, report_document)" .
						" VALUES ($report_typ_id, $first_import_id, $second_import_id, $dev_id, '$time1', ";
					if ($time2=='NULL') { $sql_code .= 'NULL'; } 
					else { $sql_code .= "'$time2'"; }
					if (isset($client_id) and $client_id!='') {
						if (preg_match("/^null$/i", $client_id)) $sql_code .= ', NULL';
						else $sql_code .= ", $client_id"; ;
					} else $sql_code .= ', NULL';
