...
sub error_handler_update {
	...
	if (defined($current_import_id)) {
		...
		if (!defined($existing_error_str) || $existing_error_str eq '') { $existing_error_str=''; } else { $existing_error_str .= "; "; }
...
sub get_rulebase_names {	# getting device-info for all devices of the current mgmt
	my $mgm_id = shift;
	my $dbdriver = shift;
