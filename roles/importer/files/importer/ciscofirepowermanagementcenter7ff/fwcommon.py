...
def get_config(config2import, full_config, current_import_id, mgm_details, limit=100, force=False, jwt=''):
    ...
    if not parsing_config_only:   # no native config was passed in, so getting it from FortiManager
        ...
        api_domain = ''
        sid = cifp_getter.login(mgm_details['user'], mgm_details['secret'],
                                mgm_details['hostname'], mgm_details['port'], api_domain)
        if sid is None:
            logger.ERROR(
    ...
    if adom_name is None:
        ...
        if not parsing_config_only:   # no native config was passed in, so getting it from FortiManager
            getObjects(sid, fm_api_url, full_config, adom_name,
                       limit, scope, nw_obj_types, svc_obj_types)
            ...
            cifp_rule.initializeRulebases(full_config)
            for dev in mgm_details['devices']:
                cifp_rule.getAccessPolicy(
                    sid, fm_api_url, full_config, adom_name, dev, limit)
                cifp_rule.getNatPolicy(
                    sid, fm_api_url, full_config, adom_name, dev, limit)
...
def getObjects(sid, fm_api_url, raw_config, adom_name, limit, scope, nw_obj_types, svc_obj_types):
    ...
    cifp_getter.update_config_with_fortinet_api_call(
        raw_config, sid, fm_api_url, "/pm/config/global/obj/user/local", "users_local", limit=limit)
