...
def import_management(
    ...
    try:
        ...
        handle_shutdown_exception(global_state, import_state, config_importer=config_importer, exc=e)
        raise
...
def _import_management(
    ...
    if fwo_config.clear:
        ...
        config_changed_since_last_import, config_normalized = get_config_top_level(
            global_state, import_state, import_state.input_file, gateways, force_import=fwo_config.force
        )
    ...
    if config_changed_since_last_import or fwo_config.force:
        if not fwo_config.suppress_consistency_check:
            FwConfigImportCheckConsistency(global_state, import_state).check_fwconfig_managerlist_consistency(
                config_normalized
            )
...
def handle_shutdown_exception(
    ...
    if import_state.rollback_required:
        roll_back_exception_handler(
            global_state,
            import_state,
            config_importer=config_importer,
    ...
    import_state.delete_import(global_state.fwo_api_call)  # delete whole import without rollback
...
def get_config_from_api(
    ...
    config_changed_since_last_import = fw_module.has_config_changed(config_in, import_state, force_import)
...
def set_filename(import_state: ImportState, file_name: str = ""):
...
def get_config_uri(import_state: ImportState) -> str | None:
