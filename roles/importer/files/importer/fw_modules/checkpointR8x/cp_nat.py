...
def get_initial_nat_rulebase_link(gateway: dict[str, Any], normalized_config: dict[str, Any]) -> dict[str, Any] | None:
    """
    Finds the link pointing at the gateway's own (local) policy.

    On a super-manager, a gateway covered by a global policy has its "is_initial" flag on the
    global link instead (see define_initial_rulebase_links in fwcommon.py), so this deliberately
    does not filter on is_initial - the local, non-global, policy-type link is unique per gateway
    regardless of which link is the RuleTreeBuilder traversal entry point.
    """
    ...
    initial_gateway_link = next(
        (
            ...
            if link.get("link_type") == "policy" and not link.get("is_global")
        ),
        None,
    )
...
def parse_native_nat_rulebases(
    ...
    for nat_rulebase in native_nat_rulebases:
        ...
        normalized_nat_rulebase = insert_parent_nat_rulebase(initial_to_rulebase_uid, import_state, normalized_config)
...
def insert_parent_nat_rulebase(
    ...
    nat_rulebase_uid = "nat-rulebase-" + policy_uid
...
def parse_nat_rule_transform(nat_rule: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    nat_in_rule = {
        "uid": nat_rule["uid"],
        "source": [nat_rule.get("original-source")],
        "destination": [nat_rule.get("original-destination")],
        "service": [nat_rule.get("original-service")],
        ...
        "install-on": nat_rule.get("install-on", []),
        "time": nat_rule.get("time"),
        "enabled": nat_rule.get("enabled", True),
        "comments": nat_rule.get("comments"),
        "nat_rule": True,
    ...
    nat_out_rule = {
        ...
        "source": [nat_rule.get("translated-source")],
        "destination": [nat_rule.get("translated-destination")],
        "service": [nat_rule.get("translated-service")],
