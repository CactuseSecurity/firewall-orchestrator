...
def process_ipv4_routing(native_config: dict[str, Any], normalized_config: dict[str, Any], dev_id: int, full_vdom_name: str) -> None:
    ...
    for route in native_config[routing_key]:
        norm_route = Route(dev_id, route['gateway'], route['ip_mask'], 
                          interface=route['interface'], metric=route['metric'], 
                          distance=route['distance'])
        normalized_config['routing'].append(norm_route)

def process_ipv6_routing(native_config: dict[str, Any], normalized_config: dict[str, Any], dev_id: int, full_vdom_name: str) -> None:
    ...
    for route in native_config[routing_key]:
        norm_route = Route(dev_id, route['gateway'], route['ip_mask'], 
                          metric=route['metric'], distance=route['distance'], 
                          interface=route['interface'], ip_version=6)
        normalized_config['routing'].append(norm_route)

def process_device_interfaces(native_config: dict[str, Any], normalized_config: dict[str, Any], dev_id: int, full_vdom_name: str) -> None:
    ...
    for interface in native_config[interfaces_key]:
        ...
        if ('ipv6' in interface and 'ip6-address' in interface['ipv6'] 
                and interface['ipv6']['ip6-address'] != '::/0'):
            ipv6, netmask_bits = interface['ipv6']['ip6-address'].split('/')
            norm_if_v6 = Interface(dev_id, interface['name'], IPAddress(ipv6), 
                                   netmask_bits, ip_version=6)
        ...
        if 'ip' in interface and interface['ip'] != ['0.0.0.0', '0.0.0.0']:
            ...
            norm_if_v4 = Interface(dev_id, interface['name'], ipv4, 
                                   netmask_bits, ip_version=4)
...
def fetch_device_interfaces(sid: str, fm_api_url: str, native_config: dict[str, Any], 
                           plain_dev_name: str, plain_vdom_name: str, full_vdom_name: str, limit: int) -> None:
    ...
    try:
        ...
        FWOLogger.warning("error while getting interfaces of device " + plain_vdom_name + 
                          ", vdom=" + plain_vdom_name + ", ignoring, traceback: " + str(traceback.format_exc()))

def fetch_routing_table(sid: str, fm_api_url: str, native_config: dict[str, Any], 
                       adom_name: str, plain_dev_name: str, plain_vdom_name: str, 
                       full_vdom_name: str, ip_version: str, limit: int) -> None:
    ...
    try:
        ...
        for item in routing_helper:
            if item['type'] == routing_key:
                ...
                if (len(routing_data) > 0 and 'response' in routing_data[0] 
                        and 'results' in routing_data[0]['response']):
                    routing_table = routing_data[0]['response']['results']
                else:
                    FWOLogger.warning("got empty " + ip_version + " routing table from device " + 
                                      full_vdom_name + ", ignoring")
                    routing_table = []
                break
    except Exception:
...
def get_interfaces_and_routing(sid: str, fm_api_url: str, native_config: dict[str, Any], 
                              adom_name: str, devices: list[dict[str, Any]], limit: int) -> None:
    ...
    for _, plain_dev_name, plain_vdom_name, full_vdom_name in device_array:
        ...
        fetch_device_interfaces(sid, fm_api_url, native_config, plain_dev_name, 
                               plain_vdom_name, full_vdom_name, limit)

        # Fetch routing information for both IP versions
        for ip_version in ["ipv4", "ipv6"]:
            fetch_routing_table(sid, fm_api_url, native_config, adom_name, 
                               plain_dev_name, plain_vdom_name, full_vdom_name, 
                               ip_version, limit)
