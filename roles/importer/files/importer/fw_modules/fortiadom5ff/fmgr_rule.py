...
def build_nat_addr_list(
    ...
    if target == "src":
        is_ipv6 = bool(native_rule.get("srcaddr6"))
    ...
    if target == "dst":
        is_ipv6 = bool(native_rule.get("dstaddr6"))
...
def _vip_nat_obj_name(vip_obj: dict[str, Any]) -> str | None:
    """
    Rebuilds the name of the auto-generated network object holding a VIP's mapped
    (translated) IP, following the naming convention used in normalize_vip_object_nat_ip.
    """
    nat_ip = vip_obj.get("obj_nat_ip")
    if not nat_ip:
        return None
    nat_ip_end = vip_obj.get("obj_nat_ip_end")
    if nat_ip_end and nat_ip_end != nat_ip:
        return f"{nat_ip}-{nat_ip_end}{NAT_POSTFIX}"
    return f"{nat_ip}{NAT_POSTFIX}"


def get_nat_translated_destination(
    is_dnat: bool,
    rule_dst_list: list[str],
    rule_dst_refs_list: list[str],
    normalized_config_adom: dict[str, Any],
    normalized_config_global: dict[str, Any],
) -> tuple[list[str], list[str]]:
    """
    For DNAT rules, resolves each VIP destination to the network object holding its
    mapped (translated) IP. Non-VIP destinations (and non-DNAT rules) pass through unchanged.
    """
    if not is_dnat:
        return list(rule_dst_list), list(rule_dst_refs_list)
    ...
    for addr, addr_ref in zip(rule_dst_list, rule_dst_refs_list, strict=False):
        vip_obj = next(
            (
                nw_obj
                for nw_obj in network_objects
                if "firewall/vip" in nw_obj.get("obj_native_type", "") and nw_obj.get("obj_name") == addr
            ),
            None,
        )
        nat_obj_name = _vip_nat_obj_name(vip_obj) if vip_obj is not None else None
        nat_obj = (
            next((nw_obj for nw_obj in network_objects if nw_obj.get("obj_name") == nat_obj_name), None)
            if nat_obj_name is not None
            else None
        )

        if nat_obj is None:
            if vip_obj is not None:
                FWOLogger.warning(f"DNAT: could not resolve translated destination network object for VIP '{addr}'")
            translated_dst_list.append(addr)
            translated_dst_refs_list.append(addr_ref)
            continue

        translated_dst_list.append(nat_obj["obj_name"])
        translated_dst_refs_list.append(nat_obj.get("obj_uid", nat_obj["obj_name"]))

    return translated_dst_list, translated_dst_refs_list
...
def parse_nat_ip(
    ...
    rule_uuid = native_rule.get("uuid")
    if not rule_uuid:
        # Without a per-rule uuid, falling back to a constant uid would make every such rule's
        # translated-IP object collapse onto the same obj_uid, cross-linking unrelated rules.
        FWOLogger.warning("NAT rule without UUID, cannot create unique translated IP object; skipping")
        return [], []

    ip_net = ipaddress.ip_network(f"{entries[0]}/{entries[1]}", strict=False)
    parsed_ip = str(ip_net)
    uid = f"{rule_uuid}_Translated_IP"
    normalized_config_adom["network_objects"].append(
        create_network_object(
            name=parsed_ip,
            obj_type="network",
...
def parse_nat_rules_in_rulebase(
    ...
    for native_rule in rulebase_to_parse.get("data", []):
        ...
        translated_dst_list, translated_dst_refs_list = get_nat_translated_destination(
            is_dnat, rule_dst_list, rule_dst_refs_list, normalized_config_adom, normalized_config_global
        )
