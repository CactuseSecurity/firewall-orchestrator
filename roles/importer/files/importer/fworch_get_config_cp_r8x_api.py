...
def collect_uids_from_rulebase(rulebase, debug_text):
    ...
    if 'layerchunks' in rulebase:
        ...
        #     print ("rulebase' in rulebase")
        #     for rule in rulebase['rulebase']:
        #         collect_uids_from_rule(rule, debug_text + ', ' + "calling collect_uids_from_rule - else")
        # elif 'rule-number' in rulebase:
        #     print ("'rule-number' in rulebase")
        #     # print ("else if : found no layerchunks in rulebase" + debug_text)
        #     for rule in rulebase['rulebase']:
        #         collect_uids_from_rule(rule, debug_text + ', ' + "calling collect_uids_from_rule - else")
        # else:
        #     print ("found nothing")
        #     print ("rulebase: " + rulebase['uid'])
...
if (mode=='get'):
    ...
    config_json += "]\n" # 'level': 'objects'\n"
    config_json += "}\n" # 'level': 'top'"
    with open(config_out_filename, "w") as configfile_json:
...
### enrich mode ##############################################################
    ...
    with open(config_out_filename, "r") as json_data:
        config = json.load(json_data)
    if os.path.exists(config_out_filename): # delete json file (to later write it again)
        os.remove(config_out_filename)
