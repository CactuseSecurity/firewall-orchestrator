...
class FwConfigImportObject:
    ...
    def update_objects_via_api(
        ...
        if FWOLogger.is_debug_level(9):
            json.dump(
                query_variables,
                open(  # noqa: SIM115
                    f"/usr/local/fworch/tmp/import/mgm_id_{import_state.mgm_details.mgm_id}_query_variables.json",
                    "w",
                ),
                indent=4,
            )
    ...
    def remove_outdated_memberships(
        ...
        try:
            import_result = import_state.fwo_api_call.call(import_mutation, query_variables, analyze_payload=True)
            if "errors" in import_result:
                FWOLogger.exception(
