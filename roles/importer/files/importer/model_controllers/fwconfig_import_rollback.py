        ...
        try:
            ...
            if 'errors' in rollback_result:
                FWOLogger.exception("error while trying to roll back current import for mgm id " +
                                str(import_state.mgm_details.mgm_id) + ": " + str(rollback_result['errors']))
            else:
                FWOLogger.info("import " + str(import_state.import_id) + " has been rolled back successfully")

        except Exception:
            FWOLogger.exception(f"failed to rollback current importfor mgm id {str(import_state.mgm_details.mgm_id)}: {str(traceback.format_exc())}")
