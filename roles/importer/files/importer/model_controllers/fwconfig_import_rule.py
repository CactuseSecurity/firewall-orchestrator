...
class FwConfigImportRule:
    ...
    def get_rule_refs(
        ...
        if is_prev:
            nwobj_resolveds = list(
                self.prev_group_flats_mapper.get_network_object_flats([ref[0] for ref in froms + tos])
            )
            svc_resolveds = list(self.prev_group_flats_mapper.get_service_object_flats(svcs))
            user_resolveds = list(self.prev_group_flats_mapper.get_user_flats(users))
        else:
            nwobj_resolveds = list(self.group_flats_mapper.get_network_object_flats([ref[0] for ref in froms + tos]))
            svc_resolveds = list(self.group_flats_mapper.get_service_object_flats(svcs))
            user_resolveds = list(self.group_flats_mapper.get_user_flats(users))
