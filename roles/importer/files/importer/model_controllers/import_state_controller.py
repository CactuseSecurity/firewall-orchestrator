...
class ImportStateController:
    ...
    def __str__(self):
        return f"{str(self.state.mgm_details)}(import_id={self.state.import_id})"
    ...
    def getPastImportInfos(self):
        try: # get past import details (LastFullImport, ...):
            ...
            self.state.last_full_import_id, self.state.last_full_import_date = \
                self.api_call.get_last_complete_import({"mgmId": int(self.state.mgm_details.mgm_id)}) 
        except Exception:
            FWOLogger.error(f"import_management - error while getting past import details for mgm={str(self.state.mgm_details.mgm_id)}: {str(traceback.format_exc())}")
            raise
    ...
    def set_rulebase_map(self) -> None:
        ...
        FWOLogger.debug(f"updated rulebase map for mgm_id {self.state.mgm_details.current_mgm_id} with {len(self.state.rulebase_map)} entries")
    ...
    def delete_import(self):
        ...
        try:
            result = self.api_connection.call(delete_import_mutation, query_variables={"importId": self.state.import_id})
            _ = result['data']['delete_import_control']['affected_rows']
            FWOLogger.info(f"removed import with id {str(self.state.import_id)} completely")
        except Exception:
            FWOLogger.exception(
                "fwo_api: failed to unlock import for import id " + str(self.state.import_id))
