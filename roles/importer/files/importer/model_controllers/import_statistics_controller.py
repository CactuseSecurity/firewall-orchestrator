...
class ImportStatisticsController:
    ...
    def collect_nw_obj_change_details(self, result: dict[str, int]):
        if self.statistics.network_object_add_count > 0:
            result["network_object_add_count"] = self.statistics.network_object_add_count
        if self.statistics.network_object_delete_count > 0:
            result["network_object_delete_count"] = self.statistics.network_object_delete_count
        if self.statistics.network_object_change_count > 0:
            result["network_object_change_count"] = self.statistics.network_object_change_count

    def collect_svc_obj_change_details(self, result: dict[str, int]):
        if self.statistics.service_object_add_count > 0:
            result["service_object_add_count"] = self.statistics.service_object_add_count
        if self.statistics.service_object_delete_count > 0:
            result["service_object_delete_count"] = self.statistics.service_object_delete_count
        if self.statistics.service_object_change_count > 0:
            result["service_object_change_count"] = self.statistics.service_object_change_count

    def collect_usr_obj_change_details(self, result: dict[str, int]):
        if self.statistics.user_object_add_count > 0:
            result["user_object_add_count"] = self.statistics.user_object_add_count
        if self.statistics.user_object_delete_count > 0:
            result["user_object_delete_count"] = self.statistics.user_object_delete_count
        if self.statistics.user_object_change_count > 0:
            result["user_object_change_count"] = self.statistics.user_object_change_count
    ...
    def collect_rule_change_details(self, result: dict[str, int]):
        if self.statistics.rule_add_count > 0:
            result["rule_add_count"] = self.statistics.rule_add_count
        ...
        if self.statistics.rulebase_link_add_count > 0:
            result["rulebase_link_add_count"] = self.statistics.rulebase_link_add_count
        if self.statistics.rulebase_link_delete_count > 0:
            result["rulebase_link_delete_count"] = self.statistics.rulebase_link_delete_count

    def collect_inconsistent_deletion_details(self, result: dict[str, int]):
        if self.statistics.inconsistent_nwobj_delete_count > 0:
            result["inconsistent_network_object_delete_count"] = self.statistics.inconsistent_nwobj_delete_count
        if self.statistics.inconsistent_svcobj_delete_count > 0:
            result["inconsistent_service_object_delete_count"] = self.statistics.inconsistent_svcobj_delete_count
        if self.statistics.inconsistent_userobj_delete_count > 0:
