...
class RulebaseLinkController:
    ...
    def get_rulebase_links(self, import_state_controller: ImportStateController) -> None:
        gw_ids = import_state_controller.state.lookup_all_gateway_ids()
        if len(gw_ids) == 0:
            FWOLogger.warning(
        ...
        links = import_state_controller.api_call.call(query, query_variables=query_variables)
        if "errors" in links:
    ...
    def set_map_of_all_enforcing_gateway_ids_for_rulebase_id(self, import_state_controller: ImportStateController):
        self.get_rulebase_links(import_state_controller)
