...
class TestInsertParentNatRulebase:
    def test_creates_nat_rulebase_when_missing(self, import_state_controller: ImportStateController):
        ...
        result = insert_parent_nat_rulebase("policy-1", import_state_controller.state, normalized_config)

        assert result.uid == "nat-rulebase-policy-1"
    ...
    def test_returns_existing_nat_rulebase_without_duplicate(self, import_state_controller: ImportStateController):
        existing = Rulebase(uid="nat-rulebase-policy-2", name="NAT", mgm_uid="mgm-uid-1")
        normalized_config = {"policies": [existing]}

        result = insert_parent_nat_rulebase("policy-2", import_state_controller.state, normalized_config)
...
class TestGetInitialNatRulebaseLink:
    ...
    def test_returns_none_when_no_local_policy_link(self):
        # Only a global policy link is present (e.g. a super-manager gateway fully covered by a
        # global policy has no local policy link at all), so there is nothing to match.
        gateway = {"uid": "gw-1"}
        normalized_config = self._make_normalized_config_with_gateway(
            "gw-1",
            [
                {"is_initial": True, "link_type": "policy", "is_global": True, "to_rulebase_uid": "rb-global"},
    ...
    def test_skips_global_link_on_super_manager(self):
        """
        On a super manager (MDS), a gateway covered by a global policy has its local policy
        link marked is_initial=False (the global link is the RuleTreeBuilder entry point
        instead - see define_initial_rulebase_links in fwcommon.py). The local link must still
        be found here, since it identifies the gateway's own NAT rulebase.
        """
        gateway = {"uid": "gw-1"}
        normalized_config = self._make_normalized_config_with_gateway(
            "gw-1",
            [
                {"is_initial": True, "link_type": "policy", "is_global": True, "to_rulebase_uid": "rb-global"},
                {"is_initial": False, "link_type": "policy", "is_global": False, "to_rulebase_uid": "rb-local"},
            ],
        )
        ...
        assert result is not None
        assert result["to_rulebase_uid"] == "rb-local"
...
class TestParseNativeNatRulebases:
    ...
    def test_happy_path_links_and_parses_rules(self, import_state_controller: ImportStateController):
        ...
        assert nat_rulebase.uid == "nat-rulebase-rb-access"
        ...
        assert any(
            ...
            and link.get("to_rulebase_uid") == "nat-rulebase-rb-access"
...
class TestNormalizeNatRules:
    ...
    def test_does_not_leak_nat_rules_between_gateways_on_different_policies(
        ...
        nat_rulebase_gw1 = next(rb for rb in normalized_config["policies"] if rb.uid == "nat-rulebase-policy-1")
        nat_rulebase_gw2 = next(rb for rb in normalized_config["policies"] if rb.uid == "nat-rulebase-policy-2")
    ...
    def test_shares_one_parent_nat_rulebase_across_gateways_on_the_same_policy(
        self, import_state_controller: ImportStateController
    ):
        # Two gateways installed with the same NAT policy see the same section-less NAT rule
        # uid. If each gateway got its own parent NAT rulebase keyed by gateway uid, the rule
        # would be stored twice under the identical uid, and the uid-keyed dicts further down
        # the import pipeline would collapse the two entries, silently dropping one gateway's
        # NAT rules.
        shared_rule = _make_nat_rule("shared-rule")
        shared_rule["rule-number"] = 1
        native_config: dict[str, Any] = {
            "nat_rulebases": [{"nat_rule_chunks": [{"rulebase": [shared_rule]}], "policy_uid": "policy-shared"}],
            "gateways": [{"uid": "gw-1"}, {"uid": "gw-2"}],
            "policies": [],
        }
        normalized_config: dict[str, Any] = {
            ...
            "gateways": [
                {
                    "Uid": "gw-1",
                    "RulebaseLinks": [{"is_initial": True, "link_type": "policy", "to_rulebase_uid": "policy-shared"}],
                },
                {
                    "Uid": "gw-2",
                    "RulebaseLinks": [{"is_initial": True, "link_type": "policy", "to_rulebase_uid": "policy-shared"}],
        ...
        nat_rulebases = [rb for rb in normalized_config["policies"] if rb.uid == "nat-rulebase-policy-shared"]
        assert len(nat_rulebases) == 1
        assert set(nat_rulebases[0].rules.keys()) == {"shared-rule", "shared-rule_translated"}

        for gw in normalized_config["gateways"]:
            assert any(
                link.get("to_rulebase_uid") == "nat-rulebase-policy-shared" and link.get("link_type") == "nat"
