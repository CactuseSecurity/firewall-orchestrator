...
class TestFwConfigImportRuleOrder(unittest.TestCase):
    ...
    def test_initialize_on_consecutive_insertions(self):
        ...
        rule_1_2_uid = insert_rule_in_config(self._normalized_config, rulebase.uid, 0, rule_uids, self._config_builder)
        rule_1_3_uid = insert_rule_in_config(self._normalized_config, rulebase.uid, 0, rule_uids, self._config_builder)

        # Inserting three new rules in the middle of the rulebase
        rule_1_6_uid = insert_rule_in_config(self._normalized_config, rulebase.uid, (len(rule_uids) - 3)//2, rule_uids, self._config_builder)
        rule_1_7_uid = insert_rule_in_config(self._normalized_config, rulebase.uid, (len(rule_uids) - 3)//2, rule_uids, self._config_builder)
        rule_1_8_uid = insert_rule_in_config(self._normalized_config, rulebase.uid, (len(rule_uids) - 3)//2, rule_uids, self._config_builder)

        # Inserting three new rules at the end of the rulebase
        rule_1_17_uid = insert_rule_in_config(self._normalized_config, rulebase.uid, len(rule_uids), rule_uids, self._config_builder)
        rule_1_18_uid = insert_rule_in_config(self._normalized_config, rulebase.uid, len(rule_uids), rule_uids, self._config_builder)
        rule_1_19_uid = insert_rule_in_config(self._normalized_config, rulebase.uid, len(rule_uids), rule_uids, self._config_builder)
    ...
    def test_initialize_on_move_across_rulebases(self):
        ...
        self.assertTrue(self._get_rule(1, deleted_rule.rule_uid).rule_num_numeric == rule_num_numeric_steps, f"Moved rule_num_numeric is {self._normalized_config.rulebases[1].Rules[deleted_rule.rule_uid].rule_num_numeric}, expected {rule_num_numeric_steps}")
    ...
    def test_update_rulebase_diffs_on_moves_to_beginning_middle_and_end_of_rulebase(self):
        ...
        beginning_rule_uid = move_rule_in_config(self._normalized_config, rulebase.uid, (len(rule_uids) - 1)//2, 0, rule_uids)  # Move to beginning
        middle_rule_uid = move_rule_in_config(self._normalized_config, rulebase.uid, 1, (len(rule_uids) - 1)//2, rule_uids)  # Move to middle
        end_rule_uid = move_rule_in_config(self._normalized_config, rulebase.uid, 2, len(rule_uids) - 1, rule_uids)  # Move to end
        ...
        self.assertTrue(self._get_rule(0, middle_rule_uid).rule_num_numeric == 5 * rule_num_numeric_steps / 2, f"Middle moved rule_num_numeric is {self._normalized_config.rulebases[0].Rules[middle_rule_uid].rule_num_numeric}, expected {5 * rule_num_numeric_steps / 2}")
        self.assertTrue(self._get_rule(0, end_rule_uid).rule_num_numeric == 10 * rule_num_numeric_steps, f"End moved rule_num_numeric is {self._normalized_config.rulebases[0].Rules[end_rule_uid].rule_num_numeric}, expected {10 * rule_num_numeric_steps}")
