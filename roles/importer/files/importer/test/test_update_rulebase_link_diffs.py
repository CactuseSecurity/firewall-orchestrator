...
from test.tools.set_up_test import update_rb_links, update_rule_map_and_rulebase_map, update_rule_num_numerics
...
class TestUpdateRulebaseLinkDiffs(unittest.TestCase):
    ...
    def test_add_inline_layer(self):
        ...
        from_rule_id, to_rulebase_id, from_rulebase_id = self.set_up_inline_layer_test(self._normalized_config)
    ...
    def test_delete_inline_layer(self):
        ...
        _, _, _ = self.set_up_inline_layer_test(self._previous_config)
    ...
    def set_up_inline_layer_test(self, config):

        last_rulebase = config.rulebases[-1]
        last_rulebase_last_rule_uid = list(last_rulebase.Rules.keys())[-1]
        last_rulebase_last_rule = last_rulebase.Rules.pop(last_rulebase_last_rule_uid)

        _, new_rulebase_uid = self._config_builder.add_rulebase(config, self._mgm_uid)
        ...
        from_rule_id = self._import_state.lookupRule(new_last_rulebase_last_rule_uid)
        to_rulebase_id = self._import_state.lookupRulebaseId(new_rulebase_uid)
        from_rulebase_id = self._import_state.lookupRulebaseId(last_rulebase.uid)
