                    ...
                    if (device.Rules != null && device.Rules.Length > 0)
                    {
                        ...
                        foreach (Rule rule in device.Rules)
                        {
                            if (string.IsNullOrEmpty(rule.SectionHeader))
                            {
                                ...
                                report.AppendLine($"<td>{ruleDisplay.DisplaySource(rule, "", ReportType)}</td>");
                                report.AppendLine($"<td>{ruleDisplay.DisplayDestinationZone(rule)}</td>");
                                report.AppendLine($"<td>{ruleDisplay.DisplayDestination(rule, "", ReportType)}</td>");
                                report.AppendLine($"<td>{ruleDisplay.DisplayService(rule, "", ReportType)}</td>");
