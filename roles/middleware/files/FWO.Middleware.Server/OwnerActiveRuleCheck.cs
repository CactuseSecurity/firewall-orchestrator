...
namespace FWO.Middleware.Server
{
    ...
    public class OwnerActiveRuleCheck(ApiConnection apiConnection, GlobalConfig globalConfig) : RuleNotificationBodyBase(globalConfig)
    {
        ...
        private async Task<int> CheckActiveRules(FwoOwner owner, NotificationDeadline deadline)
        {
            ...
            {
                string? htmlBody = notification.Layout == NotificationLayout.HtmlInBody
                    ? CreateNotificationBodyHtml(GlobalConfig.GetText("OwnerActiveRules"), body, owner.Name)
                    : null;
                ...
                emailsSent += deadline == NotificationDeadline.None
                    ? await notificationService.SendNotification(notification, owner, notificationContent, null, "", htmlBody)
                    : await notificationService.SendNotificationIfDue(notification, owner, null, notificationContent, null, "", htmlBody);
            }
