...
{
    /// <summary>
    /// Quartz scheduler service for app data imports
    /// </summary>
    public class ImportAppDataSchedulerService : BackgroundService
    {
        ...
        public ImportAppDataSchedulerService(ISchedulerFactory schedulerFactory, ApiConnection apiConnection, GlobalConfig globalConfig)
        {
        ...
        private async Task ScheduleJob()
        {
            ...
            var job = JobBuilder.Create<ImportAppDataJob>()
                .WithIdentity(jobKey)
                .Build();

            var startTime = CalculateStartTime(globalConfig.ImportAppDataStartAt, TimeSpan.FromHours(globalConfig.ImportAppDataSleepTime));
            ...
            Log.WriteInfo(SchedulerName, $"Job scheduled. Start: {startTime:yyyy-MM-dd HH:mm:ss}, Interval: {globalConfig.ImportAppDataSleepTime}h");
        }
