...
namespace FWO.Test
{
    internal class ExtTicketHandlerTestApiConn : SimulatedApiConnection
    {
        ...
        public bool ApprovalPhaseActive { get; set; }
        public bool ApprovalPhaseActiveForMasterOnly { get; set; }
        public bool ApprovalPhaseActiveForTaskTypesOnly { get; set; }
        public bool ReturnNullOnFirstTicketQuery { get; set; }
        ...
        public override async Task<QueryResponseType> SendQueryAsync<QueryResponseType>(string query, object? variables = null, string? operationName = null, FWO.Api.Client.QueryChunkingOptions? chunkingOptions = null)
        {
            ...
            else if (responseType == typeof(List<OwnerResponsibleType>))
            {
                GraphQLResponse<dynamic> response = new() { Data = new List<OwnerResponsibleType>() };
                return response.Data;
            }
            else if (responseType == typeof(List<UiUser>))
            {
            ...
            {
                WfTaskType taskType = WfTaskType.master;
                ...
                if (ApprovalPhaseActive
                    || (ApprovalPhaseActiveForMasterOnly && taskType == WfTaskType.master)
                    || (ApprovalPhaseActiveForTaskTypesOnly && taskType != WfTaskType.master))
                {
                    foreach (WorkflowConfigurationPhase phase in configurations.SelectMany(configuration => configuration.Phases)
                        .Where(phase => phase.Phase == WorkflowPhases.approval.ToString()))
                    {
                        phase.PhaseMatrix.Active = true;
                    }
                }
