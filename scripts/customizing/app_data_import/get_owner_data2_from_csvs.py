...
def extract_app_data_from_csv_file(csv_file: str, app_data: dict[str, dict[str, Any]], contains_ip: bool) -> None:
    app_name_column, app_id_column, app_owner_biso_column, app_owner_tiso_column, app_server_ip_column = (
        _get_csv_columns(contains_ip)
    )
    ...
    for line in app_data_from_csv:
        ...
        if not contains_ip:
            if not _add_app_from_line(
                ...
                logger.debug("ignoring line from csv file: %s - inactive?", app_id)
                count_skips += 1
...
if __name__ == "__main__":
    ...
    for csv_file in csv_all_owner_files:
        extract_app_data_from_csv_file(csv_file, app_data, contains_ip=False)
    for csv_file in csv_app_server_files:
        extract_app_data_from_csv_file(csv_file, app_data, contains_ip=True)

    owner_data: dict[str, list[dict[str, Any]]] = {"owners": []}

    app_id: str
    for app_id, app_info in app_data.items():
        if app_info["app_id_external"] != "":
            owner_data["owners"].append(
                {
                    ...
                    "app_servers": app_info["app_servers"],
                }
            )
        else:
            logger.warning("App %s has no external app id, skipping...", app_id)
    ...
    #############################################
    # 4. Some statistics
    logger.info("total #apps: %s", len(app_data))
    apps_with_ip: int = 0
