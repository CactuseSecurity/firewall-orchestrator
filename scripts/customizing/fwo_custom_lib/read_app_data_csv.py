...
DEFAULT_IP_HEADER_PATTERNS: dict[str, str] = {
    "app_id": r'.*?:\s*Alfabet-ID$',
    "ip": r'.*?:\s*IP'
}
...
def parse_ip(
    ...
    if app_server_ip_string is not None and app_server_ip_string != "":
        ...
        app_server_ip: Appip = app_ip_cls(app_id_external=app_id, ip_start=ip_start, ip_end=ip_end, type=ip_type, name=ip_obj_name)
