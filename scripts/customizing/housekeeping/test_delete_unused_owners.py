...
def fake_fetch_blocked_candidate(*_: object) -> list[module.InactiveOwnerCandidate]:
    return [module.parse_owner_candidate(build_owner_payload(5, {"reports": 1}))]


def fake_fetch_mixed_candidates(*_: object) -> list[module.InactiveOwnerCandidate]:
    return [
...
def build_owner_payload(
    ...
    *,
    ...
    counts: dict[str, int] = dict.fromkeys(module.OWNER_REFERENCE_RELATIONS, 0)
...
def test_build_inactive_owners_query_without_owner_filter_excludes_owner_ids_variable() -> None:
    query = module.build_inactive_owners_query(filter_by_owner_ids=False)
...
def test_build_inactive_owners_query_with_owner_filter_includes_owner_ids_variable() -> None:
    query = module.build_inactive_owners_query(filter_by_owner_ids=True)
